      ...
      {acceptedFileTypes.map((item, index) => (
        <Card
          ...
          className={cn(
            ...
            index === 1 && "z-10",
            isDragging &&
              "bg-[#1A201F]/60 text-white/85 shadow-md shadow-black/10 dark:shadow-black/25 dark:bg-[#1A201F]/60 dark:text-white/85"
          )}
          style={{
...
export function FileUpload({
  ...
  const dropzone = (
    <div
      ...
      tabIndex={0}
      className={cn(
        "relative flex min-h-64 cursor-pointer flex-col items-center justify-center gap-5 overflow-hidden rounded-[1.125rem] border border-white/10 border-dashed bg-white/5 px-6 py-10 text-center transition-[border-color,background-color] duration-200 ease-out dark:border-white/10 dark:bg-[#1A201F]/80",
        "motion-reduce:transition-none",
        isDragging
          ? "border-white/10/40 bg-white/5/35 dark:border-white/10/40 dark:bg-[#252b2a]/35"
          : "border-white/10/20 hover:border-white/10/35 hover:bg-white/5/35 dark:border-white/10/25 dark:hover:border-white/10/40 dark:border-white/10/20 dark:hover:border-white/10/35 dark:hover:bg-[#252b2a]/35 dark:dark:border-white/10/25 dark:dark:hover:border-white/10/40"
      )}
    ...
    >
      ...
      <div className="space-y-1">
        ...
        <div className="text-xs text-white/50 dark:text-white/50">{description}</div>
        {rejectionMessage ? (
          <div className="text-xs text-red-400 dark:text-red-400">{rejectionMessage}</div>
        ) : null}
      </div>
      <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/50 dark:border-white/10 dark:bg-[#1A201F]/80 dark:text-white/50">
  ...
  return (
    <div className={cn("space-y-3", className)}>
      ...
      {showFileList && files.length > 0 ? (
        <div className="rounded-xl border border-white/10 bg-white/5 dark:border-white/10 dark:bg-[#1A201F]/80">
          {files.map((file) => (
            <div
              key={file.id}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-xs text-white/50 dark:text-white/50">
                  {file.type} - {formatBytes(file.size)}
                </div>
              </div>
              <div className="rounded-full bg-white/5 px-2 py-1 text-xs text-white/50 dark:bg-[#252b2a] dark:text-white/50">
                Ready
              </div>
            </div>
          ))}
