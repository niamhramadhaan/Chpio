...
export function FileThumbnailLoadingOverlay() {
  return (
    <div
      aria-hidden="true"
      className="absolute inset-0 z-10 overflow-hidden bg-white/5 dark:bg-[#252b2a]"
    >
      <div className="absolute inset-0 bg-white/5 dark:bg-[#252b2a]" />
      <div className="absolute inset-0 animate-pulse bg-white/5/55 motion-reduce:animate-none dark:bg-[#1A201F]/80/55" />
    </div>
  )
}
...
export function FileThumbnail({
  ...
  return (
    <div
      className={cx(
        "group overflow-hidden rounded-lg border border-white/10 bg-white/5 text-white/85 dark:border-white/10 dark:bg-[#1A201F]/80 dark:text-white/85",
        className
      )}
    >
      <div
        className={cx(
          "relative aspect-square overflow-hidden bg-white/5 [contain:layout_paint] dark:bg-[#252b2a]",
          previewClassName
        )}
        style={
          previewAspectRatio
      ...
      >
        ...
        {showFallback ? (
          <div className="absolute inset-0 bg-white/5 dark:bg-[#252b2a]" aria-hidden="true" />
        ) : null}
      </div>
    </div>
  )
